...
const idsRequest = axios.create({
    baseURL: process.env.NODE_ENV === 'production' ? config.dev.baseUrl.ids : config.prod.baseUrl.ids
})

idsRequest.interceptors.request.use(
    function (config) {
        config = {
            ...config,
...
idsRequest.interceptors.response.use(
    function (response) {
        return response
    },
    function (error) {
