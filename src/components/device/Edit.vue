...
export default defineComponent({
  props: {
    editing: Boolean,
    device: { type: Object as PropType<IDevice>, required: true },
  },
  setup(props, { emit }) {
    ...
    const updateData = () => {
      ...
      const url = `${config.api.device_category}/${formState.value.id}`;
      const data = {
        code: formState.value.code,
        name: formState.value.name,
        os: formState.value.os,
      ...
      apiRequest
        ...
        .then((res: any) => {
          ...
          getBase64(data.file, (base64Url: string) => {
            ...
            formState.value.imageUrl = base64Url;
            loading.value = false;
          });
        })
        .catch((err) => {
