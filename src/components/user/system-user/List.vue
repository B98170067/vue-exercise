  ...
  <a-space direction="vertical">
    <a-space>
      ...
      <a-select v-model:value="auth" style="width: 120px">
        <a-select-option value="">All</a-select-option>
        <a-select-option value="true">系統管理員</a-select-option>
        <a-select-option value="false">超級管理員</a-select-option>
      </a-select>
...
import Create from "@/components/user/system-user/Create.vue";
import Edit from "@/components/user/system-user/Edit.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const getData = (filter: any) => {
      ...
      apiRequest
        .get(config.api.register_manager, {
          params,
        })
        .then((res: any) => {
