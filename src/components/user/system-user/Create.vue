<template>
  <div>
    ...
    >
      ...
      >
        <a-form-item ref="id" label="帳號" name="id">
        ...
        <a-form-item ref="auth" label="帳號權限" name="auth">
          <a-radio-group name="auth" v-model:value="formState.auth">
  ...
  setup(props, { emit }) {
    ...
    const createData = () => {
      ...
      const data = {
        ...
        RoleName: formState.value.auth,
        ProfileKey: formState.value.profileKey,
      };

      apiRequest
        .post(config.api.register_manager, data)
        .then(() => {
          formRef.value.resetFields();
        })
        .catch((err) => {
